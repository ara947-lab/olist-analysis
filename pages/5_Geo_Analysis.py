...
def load_geo_data():
    ...
    geo_avg = pd.read_csv(os.path.join(data_dir, "geo_preprocessed.csv"), encoding="latin-1")
    
    # 2. ?Рызд??ы░?ъ╡мызд???░ьЭ┤??ыбЬыУЬ
    df_sellers = pd.read_csv(os.path.join(data_dir, "olist_sellers_dataset.csv"), encoding="latin-1")
    df_customers = pd.read_csv(os.path.join(data_dir, "olist_customers_dataset.csv"), encoding="latin-1")
